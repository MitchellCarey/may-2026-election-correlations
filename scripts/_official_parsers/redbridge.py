"""Parse Redbridge Council 2026 ward results from redbridge.gov.uk.

Each ward block opens with `<h2>WardName</h2>` followed by a 3-column
table (Candidate | Party | Votes). The elected candidate's row wraps
every cell's contents in `<strong>...</strong>` and the votes cell text
ends with " Elected" (e.g. `<strong>1729 Elected</strong>`). Candidate
cells split surname/forename across a `<br>`.

Multi-seat wards (Aldborough returned three Labour councillors in 2026)
are handled by `pick_plurality` over every elected row in the ward.
"""
import html
import re
import urllib.parse
