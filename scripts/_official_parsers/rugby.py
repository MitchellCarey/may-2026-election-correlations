"""Parse Rugby Borough Council 2026 ward results from rugby.gov.uk.

Rugby's Liferay-served `/w/2026-elections` page hides each ward inside
`<h2 class="section_title">WardName Ward</h2>` followed by a 3-column
table (Name | Party | Votes). The elected candidate's row wraps every
cell's contents in `<strong>...</strong>` and the votes cell ends with
the literal text " elected" (e.g. `<strong>856 elected</strong>`).
"""
import html
import re
import urllib.parse
...
WARD_HEAD_RE = re.compile(
    r'<h2[^>]*class="section_title"[^>]*>\s*([^<]+?Ward)\s*</h2>',
    re.IGNORECASE,
)
...
def parse(content: bytes, *, council: dict, year: int) -> list[dict]:
    ...
    for i, m in enumerate(ward_heads):
        ...
        # H2 heads carry a literal " Ward" suffix; strip it so the name
