"""Parse Sheffield City Council 2026 ward results from sheffield.gov.uk.

Each ward block opens with `<h3 id="{slug}">WardName</h3>` followed by a
3-column table (Name | Description | Votes). The elected candidate's row
wraps every cell's contents in `<strong>...</strong>` — losing rows are
bare text. The page also restates the winner above the table in a `<p>`,
but the per-cell `<strong>` marker is the parse anchor.
...
def parse(content: bytes, *, council: dict, year: int) -> list[dict]:
    ...
    for i, m in enumerate(ward_heads):
        ...
        for rm in ROW_RE.finditer(src, block_start, block_end):
            row = rm.group(1)
            # Elected sentinel: every cell's content opens with <strong>.
