...
if (target) {
  ...
  const KIND_ORDER = { referendum: -3, 'gla-mayor': -2, 'gla-assembly': -1,
                        ced: 0, ward: 1,
...
// non-empty). Eight buttons with data-view attributes flip the SVG root
...
// unitaries), .view-eu_ref_2016 (only EU referendum LADs),
// .view-gla-mayor (only the GLA Mayor polygon), and .view-gla-assembly
// (only the 14 London Assembly Constituency polygons). DOM is built
// once; CSS does the per-mode hiding.
const VIEWS = ['recent', 'wards', 'ceds', 'holyrood', 'senedd', 'pcon',
               'surrey', 'eu_ref_2016', 'gla-mayor', 'gla-assembly'];
const setView = name => {
  document.querySelectorAll('.map-svg').forEach(svg => {
...
(function wireClicks() {
  ...
  root.addEventListener('click', evt => {
    const target = evt.target.closest(
      'path.ward, path.ced, path.holyrood, path.senedd, path.pcon, path.referendum, path.gla-mayor, path.gla-assembly');
