...
### GLA Assembly overlay (issue #89) → feeds the GB current map historical slider

```
31_fetch_gla_assembly_geom.py   → data/gla_assembly_geom.json   (14 LAC polygons E32000001..E32000014 sharing the gb viewBox; raw cache at data/source/lac_dec_2017.geojson)
29_fetch_gla_assembly_results.py → data/source/wiki_gla_assembly_<year>.json   (4 per-year articles, idempotent)
30_extract_gla_assembly.py      → data/gla_assembly_raw.json    (56 FPTP + 4 list records, kind="fptp" / kind="list")
04h_build_gla_assembly_history.py → data/gla_assembly_history.json   (constituencies + single "Greater London" region history)
07d_build_current_map_artifact.py → consumes both on --region=gb; emits GLA_ASSEMBLY / GLA_ASSEMBLY_PATHS / GLA_ASSEMBLY_HISTORY / GLA_ASSEMBLY_REGION_HISTORY / GLA_ASSEMBLY_RAW_DISPLAY constants
```

Sibling of the GLA Mayor overlay (#81) — the GLA constituency boundaries have also been stable since 2000, so the same era-less render shape applies, and the December 2017 ONS BGC vintage describes the geometry used at every contest in scope (2012/2016/2021/2024). The Assembly's electoral system is dual: 14 FPTP constituency members + 11 d'Hondt London-wide list members. Unlike Holyrood (8 regions × 7 list seats) or Senedd (5 regions × 4 list seats), the Assembly's list seats come from a single London-wide ballot, so the registry exposes one region (`"Greater London"`) and 04h's region pass produces 1 region × 4 years = 4 list records.

`31` fetches `London_Assembly_Constituencies_Dec_2017_EN_BGC_2022` (the only LAC vintage shipped with geometry on the ONS portal — the post-2017 LAC_*_NC datasets carry names and codes only). `29` fetches four per-year Wikipedia articles (`2012_London_Assembly_election`, `2016_London_Assembly_election`, `2021_London_Assembly_election`, `2024_London_Assembly_election`); the per-constituency articles (e.g. `Brent_and_Harrow_(London_Assembly_constituency)`) carry only free-text result lists and aren't parseable, so the consolidated per-year articles are the only practical Wikipedia source. `30` parses each article in two passes: (a) a constituency results wikitable where the winner's cell is bolded (`'''<name>'''`) and styled with a literal background-colour hex code (`#faa`/`#ffaaaa` = Labour, `#aacfff` = Conservative, `#ffd152` = LibDem — no constituency has been won by Green/Reform/UKIP historically, so the colour map is small); (b) a summary template carrying the 11-seat London-wide list allocation — either `{{Election results}}` (2016/2021/2024) with per-party `partyN`/`seatsN_2` field pairs, or `{{AMS Election Summary Party}}` blocks (2012, pre-2016 layout) with `party`/`AMS seats` fields.

Each polygon carries `class="gla-assembly"` + `data-lac="<LAC24CD>"`. `paintAtYear`'s `path.gla-assembly` block walks `GLA_ASSEMBLY_HISTORY[lac].history` for the newest entry with `y <= targetYear` (same shape as the mayor block), then splices a "London-wide list 2024: Lab 4, Con 3, Green 3, LibDem 1, Reform 1 (11 seats)" line into the tooltip by carry-forwarding through `GLA_ASSEMBLY_REGION_HISTORY["Greater London"]`. The label uses `seats_raw` when populated (with `GLA_ASSEMBLY_RAW_DISPLAY` shortening "Labour Party (UK)" → "Lab", "UK Independence Party" → "UKIP", etc.) so historically distinct minor parties like UKIP 2016 / Reform 2024 render under their own labels rather than collapsing to "Other".

The view picker gains a **"London Assembly only"** button (`data-view="gla-assembly"`) sitting next to "London Mayor only" — CSS-isolates the 14 LAC polygons. No new slider year stops: 2012/2016/2021/2024 are already in the union via mayor + PCON + CEDs + wards. GM is byte-identical: 07d gates both data loads on `region == 'gb'` and emits empty `GLA_ASSEMBLY` / `GLA_ASSEMBLY_PATHS` arrays for GM (the renderer's `if (… || GLA_ASSEMBLY.length)` gate keeps the view picker dormant when the arrays are empty). An optional `data/source/gla_assembly_official_<year>.csv` (schema `lac24cd,year,party,candidate,source,url,note`) is wired in at 04h's highest pass rank for future hand-curation — currently empty since `30`'s wiki extraction achieves 14/14 × 4 years (56/56) + 4/4 × 11 seats with zero gaps.
...
### European Parliament 2014 + 2019 overlay (issue #91) → feeds the GB current map historical slider

```
_eu_parliament.py              → registry of 11 GB EERs (NI excluded) + per-region Wikipedia article titles
29_fetch_eu_parliament.py      → data/source/wiki_ep_<eer_code>.json (11 articles; one fetch per EER covers both 2014 + 2019)
30_extract_eu_parliament.py    → data/ep_results.json (11 regions × 2 years = 22 region-years; per-record: w, seats, seats_raw, votes, url)
31_fetch_eer_geoms.py          → data/eer_geoms.json  (11 EER polygons in the GB SVG coord space; raw cache at data/source/eer_dec_2018_uk_bgc.geojson)
07d_build_current_map_artifact.py → consumes both on --region=gb; emits EP / EP_PATHS / EP_HISTORY / EP_RAW_DISPLAY / EP_PARTIES constants
```

The UK was divided into 12 European Electoral Regions from 1999 until Brexit on 31 January 2020 (9 English Government Office Regions + Scotland + Wales + Northern Ireland). EER boundaries did not change across this period, so a single ONS `EER_Dec_2018_GCB_UK_2022` snapshot describes the geometry in legal effect at both the 22 May 2014 (UKIP-led) and 23 May 2019 (Brexit-Party-led) contests. NI is excluded throughout: NI MEPs were elected under STV rather than d'Hondt, and the GB `gb` viewBox excludes NI anyway. 11 GB polygons × 2 years = 22 records (70 GB seats at each contest).

`_eu_parliament.py` is the single source of truth (EER code + display name + Wikipedia article title — uniform `"<Region> (European Parliament constituency)"`). Each per-constituency article carries one `=== YYYY ===` H3 per contest since 1999 inside an `== Election results ==` H2, so one fetch per region yields both target years — same pattern Holyrood (17b) and Senedd (14b) per-constituency fetchers use.

`30` parses each year's `{{Election box begin for list|title=…}}` ... `{{Election box end}}` block. Seats per party come from counting `'''Bold Name''' (rank)` spans inside each `{{Election box candidate with party link|...|party=PARTY|candidate=...|votes=N}}` row — Wikipedia's convention is that elected MEPs are shown in bold with their seat-rank in parentheses, while non-elected list candidates are shown in `<small>...</small>`. So **bold-count per party = seats won under d'Hondt**. Per-region totals: 3 NE / 8 NW / 6 Yorkshire / 5 EM / 7 WM / 7 East / 8 London / 10 SE / 6 SW / 6 Scotland / 4 Wales = 70 GB seats at each contest.

Two parser gotchas worth flagging for future maintainers:
- The body of each candidate template uses a **brace-balanced walker** (mirroring `15b_extract_senedd_history.py::iter_candidate_bodies`) because some 2019 rows nest two levels of templates inside `change=` (e.g. `change={{nowrap|{{increase}}32.46}}`). A regex with single-level nesting (the simpler pattern 15 uses for the 2026 Senedd article) silently drops those rows — Wales 2019 Brexit Party is the canonical victim.
- The bold-span pattern is `'''.+?'''` with DOTALL, **not** `'''[^']+?'''`. Candidate names contain literal apostrophes ("O'Flynn", "O'Connor") that the negated-class form treats as the closing quote and silently truncates the span, under-counting the party's seats. East of England 2014 UKIP (Patrick O'Flynn) is the canonical victim.

`31` mirrors `28_fetch_gla_mayor_geom.py` — fetches the ONS layer in EPSG:27700 GeoJSON, simplifies at 500 m (regional-scale polygons; the 100 m tolerance other overlays use produces a ~1 MB artifact for 11 EERs because of Scottish island detail), and y-flips into the shared SVG coord space via bounds re-discovered from `ward_geoms.json`. GB filter via `where="eer18cd LIKE 'E15%' OR 'S15%' OR 'W08%'"` excludes NI server-side.

Each EP history record carries both `seats` (normalised labels via `normalize_party`) and `seats_raw` (raw Wikipedia party strings) so the tooltip can render historically accurate labels — `UK Independence Party` and `Brexit Party` would otherwise be lost. `_artifact_lib.PARTY_COLOURS_MAP` gains `UKIP` (#70147A — UKIP brand purple) and `Brexit` (#00AEEF — Brexit Party brand light-blue, kept deliberately distinct from Reform UK's brighter `#12B6CF` even though Reform UK legally inherited the Brexit Party registration in 2020). The `_wiki_parser.normalize_party` rules pick up `'uk independence' in s → 'UKIP'` and `'brexit' in s → 'Brexit'`. A small side effect of these new mappings: GE 2015 Westminster results now attribute Douglas Carswell's Clacton win to UKIP rather than "Other" — a mild improvement in accuracy, not a regression.

07d stamps each `path.ep` with `data-eer` (the EER GSS code) and no `data-era` (EERs are single-vintage — no boundary review across the timeframe). `paintAtYear`'s `path.ep` block walks `EP_HISTORY[<eer>].history` for the newest entry with `y <= sliderYear` and rewrites the fill + dataset URL + tooltip every tick. Below 2014 the polygons paint grey. Tooltip rewrite is *required* on every tick (not skipped for any era like Senedd post-era is) because the seat split changes sharply between 2014 (UKIP-dominant) and 2019 (Brexit-Party-dominant) — readers need to see the layer's d'Hondt breakdown update as they scrub.

The view picker gains a **"European Parliament"** button (`data-view="ep"`) which CSS-isolates the 11 EER polygons; in every other view `path.ep` is hidden (mirroring EU Ref 2016's opt-in convention — the EERs are too large to paint chronologically in the default recent view without overdrawing wards/CEDs at regional scale). The shared legend is fed by a build-time `EP_PARTIES` constant (every party that ever won an EP seat at 2014 or 2019); UKIP / Brexit Party show their swatches when the EP layer is non-empty, even before the slider triggers paint.

This phase adds the slider year stop **2014** to the union — it's PCON-less, CED-less, devolved-less, ward-less, so at year=2014 the 11 EER polygons (in the EP view) or carry-forwarded GLA Mayor (in the default view) are the only layers with a real result. 2019 already exists in the slider via PCON history. GM is byte-identical: 07d gates both data loads on `region == 'gb'` and emits empty `EP` / `EP_PATHS` / `EP_PARTIES` arrays for GM (the view picker's existing `|| EP.length` gate keeps the button dormant when the array is empty).
...
- **Touched `scripts/29_fetch_gla_assembly_results.py` / `scripts/30_extract_gla_assembly.py` / `scripts/31_fetch_gla_assembly_geom.py` / `scripts/04h_build_gla_assembly_history.py` / `scripts/_gla_assembly.py`** → run from that script onwards. For `31` (geometry refetch): `31 --force` → `04h` → `07d` (both regions). For `29` (registry edit or new wiki_title): delete affected `data/source/wiki_gla_assembly_*.json` or pass `--force`, then `30` → `04h` → `07d` (both regions). For `30` (parser change): `30` → `04h` → `07d` (both regions). For `04h` alone (CSV fallback added): `04h` → `07d --region=gm` (byte-identical: GLA_ASSEMBLY_HISTORY only ships on `--region=gb`) → `07d --region=gb`.
- **Added or edited a row in `data/source/gla_assembly_official_<year>.csv`** → run `04h` → `07d` (both regions). The CSV is hand-curation reserved for Wikipedia gaps — currently empty since `30` achieves 14/14 × 4 years coverage off the wiki cache alone.
...
- **Touched `scripts/29_fetch_eu_parliament.py` / `scripts/30_extract_eu_parliament.py` / `scripts/31_fetch_eer_geoms.py` / `scripts/_eu_parliament.py`** → run from that script onwards through `07d --region=gm` (byte-identical: EP / EP_HISTORY only ship on `--region=gb`) and `07d --region=gb`. For `29` (registry edits or new wiki_title overrides): delete the affected `data/source/wiki_ep_<eer_code>.json`, re-run `29`, then `30` → `07d` (both regions). For `30` alone (parser change): `30` → `07d` (both regions); `30`'s stderr summary should print GB seat totals of 70 at both 2014 and 2019 (UKIP 24 / Labour 20 / Conservative 19 / Green 3 / SNP 2 / LibDem 1 / Plaid 1 at 2014; Brexit 29 / LibDem 16 / Labour 10 / Green 7 / Conservative 4 / SNP 3 / Plaid 1 at 2019). For `31 --force` (boundary geometry; idempotent and exits early when `data/eer_geoms.json` exists): `31 --force` → `07d` (both regions).
